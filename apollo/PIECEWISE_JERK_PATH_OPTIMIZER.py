# https://zhuanlan.zhihu.com/p/404088222
...
import random
# 障碍物设置
obs = [[5, 10, 2, 3], [15, 20, -2, -0.5],
       [25, 30, 0, 1]]  # start_s,end_s,l_low,l_up

s_len = 50
delta_s = 0.1
n = int(s_len / delta_s)
x = np.linspace(0, s_len, n)
up_bound = [0]*(5*n + 3)   # qp问题中的上边界，包括偏移量，一阶导，二阶导，三阶导（有两块），故一共是5n
low_bound = [0]*(5*n + 3)
s_ref = [0]*3*n
...
P_l = w_l * eye_n
P_dl = w_dl * eye_n
P_ddl = (w_ddl + 2*w_dddl/delta_s/delta_s) * eye_n - \
    2 * w_dddl / delta_s/delta_s * np.eye(n, k=-1)
